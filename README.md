...
- **React and Flutter are the same library.** Not similar and not ported in spirit: the Dart parser is the TypeScript parser, file for file, and a check in CI diffs both parsers' trees over every Markdown file here. A document that means one thing in a browser means the same thing in an app.
