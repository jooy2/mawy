...
- **No image upload in the Flutter editor.** A picture on the clipboard and a file on the device are both out of a widget's reach without a plugin, and which plugin an application uses is not this package's to choose — the reason `open` is a button the application gives a meaning to. An application with a picker stores the file itself and writes the picture with `MawyEditorHandle.insert`. Revisited when the React editor gained a picker and the rules around an upload that outlives the caret, and kept: those rules exist for a file the editor was handed, and here the editor is never handed one. Written down in `docs/*/guide/editor.md` under Images, in both languages.
- **An editor with no `anchorPrefix` making none up.** Two editors on one page with a heading in common give two elements one `id` until the application passes a prefix to each. A prefix generated from `useId` would end the collision and give every heading a name no application could link to from its own interface, and which editors share a page is something only the application knows. The same answer `MawyViewer` gives, written down on `MawyEditor.anchorPrefix` and on the API page in both languages.
- **How little raw HTML `sanitize` draws without a DOM.** A `<br>`, an `<img>` with nothing but quoted `src`, `alt`, `width`, `height` and `title` whose values use no reference past `&amp;`, `&lt;`, `&gt;`, `&quot;`, `&apos;` and `&#39;` — an `&` that begins no reference, such as the one in `?x=1&y=2`, is read as itself — and a small set of attribute-less tags around some words. Everything else waits for `DOMParser`, which a server has not got, and is characters on a server. Each piece added to that set has to be something a browser has exactly one way to read — an unquoted value, a legacy reference with no semicolon, an `&` whose meaning takes the table of reference names to settle (`&not_x` is `¬_x` and `&utm_source` is not), or one attribute more is where a hand-written reader starts disagreeing with the browser about what a picture's address is. The reason is in `plainHtml` and `pairsIn` in `render.tsx` and under Safety in `docs/*/guide/viewer.md`.
- **A bare `<br>` in a table cell drawn as a line break under every `html` policy**, `escape` included, and in the Flutter package, which draws every other piece of raw HTML as its characters. A row of a table is one line of the file, so a cell has no other way to hold a second line, and `<br>` is what every GitHub table writes for it; it has no attributes to be unsafe with. The editor's `Enter` in a cell writes one. Anywhere else a `<br>` is raw HTML and the policy decides. The reason is on `cellContents` in `render.tsx` and `_cellContents` in `render.dart`, and in `docs/*/guide/viewer.md` beside the policies.
- **A line of a table cell written as a list item drawn as one**, in both packages. `- dig<br>  - deeper` in a cell is words to every parser and dashes on GitHub, and this library draws a bullet, a checkbox or the number in place of each marker, nested by the spaces in front of it. A cell holds no block, so a list is the one thing an author writing lines like that cannot have, and the editor's list commands write them. The parser's trees stay words, so the parity check says nothing about it; `tool/drawn.json` has the case. The reason is on `CellItem` in `render.tsx` and `_CellItem` in `render.dart`, and in `docs/*/guide/viewer.md` under Supported syntax.
...
- **Writing an image's dimensions in the document.** `--mawy-doc-image-aspect` is the answer for now: an application that knows what shape its pictures are reserves the box, and nothing is reserved otherwise. A syntax the parser reads, `![alt](url =800x600)`, may come later and would want the same syntax in both parsers and a line in the parity corpus. Revisited for an application moving from an editor that saved a resized picture as `<img width height>`, and still declined: that markup is drawn under `sanitize` in a browser and, since `plainHtml`, on a server, and GitHub draws it too, so it is already the way to give one picture a size that means the same thing everywhere a React page reads it. `![alt](url =800x600)` is not an image to CommonMark or to GitHub — the destination is followed by something that is not a title, so the whole run is text — and a document written with it would stop having a picture in it anywhere but here. The reasoning is in `docs/*/guide/viewer.md` beside `--mawy-doc-image-aspect`.
- **A list given up in the middle of itself left loose until something is typed there.** `Enter` on an empty item between two others takes the marker away and leaves the blank line, and one blank line does not end a list — `- one`, nothing, `- two` is one loose list to CommonMark, and that is what the preview, the viewer and GitHub all draw. While the caret is on that line the drawn surface parts the list into the two and the empty paragraph the first letter typed will make, so there is somewhere to type; a caret that leaves without typing leaves the source as it is, and the list is drawn loose, which is a wider gap than it had. Taking the blank line back on its own would be an edit made by a caret moving, with the caret it moved to needing to be shifted under it and a step in the undo history nobody asked for. `Backspace` and `Delete` on that paragraph put the list back together instead, and so does undo. The reason is on `betweenItems` in `editing.ts` and in `docs/*/guide/editor.md`, in both languages.

- **No underline command.** Markdown has no underline, and the only way to write one is `<u>`, which is raw HTML: `html="escape"`, the default, draws it as its characters, and so does the Flutter package, which has no HTML to draw. A button that wrote it would make documents that mean one thing in a viewer set to `sanitize` and another in every other place the same file is read, which is the failure this library is built to prevent. A `<u>` already in a document is drawn under `sanitize` and `raw`. Written down in `docs/*/guide/editor.md` under Formatting.
