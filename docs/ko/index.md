...
Mawy는 아직 초기 개발 단계이고, 그중 절반은 동작합니다. **파서와 뷰어가 있습니다.** CommonMark와 GitHub의 확장을 직접 만든 파서가 읽고, `MawyViewer`가 그 결과를 그립니다 — 조판 툴바, 제목 목록 패널, 라이트·다크, 그리고 문서를 아예 받지 못했을 때를 위한 파일 선택기까지. [뷰어 페이지](./guide/viewer)의 데모가 바로 그 컴포넌트가 도는 모습입니다.

다음은 에디터입니다. npm에는 아직 아무것도 올라가 있지 않고 API도 확정되지 않았습니다.
