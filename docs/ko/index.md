...
`wysiwyg` 화면 — 그려진 문서를 그 자리에서 편집하는 것 — 은 일부만 만들어졌습니다. 글자가 들어갈 수 있는 곳이면 어디서나 입력할 수 있고, 이미지와 보여주는 대신 그려지는 원시 HTML은 어설프게 되는 대신 거절됩니다. npm에는 아직 아무것도 올라가 있지 않고 API도 확정되지 않았습니다.
