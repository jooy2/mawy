...
React는 세지 않았습니다. 애플리케이션에 이미 있기 때문입니다. `lucide-react`는 셌습니다. 이 패키지를 따라 들어오기 때문입니다. **문서를 읽기만 하는 페이지는 에디터를 싣지 않습니다.** 툴바도, 되돌리기 기록도, 붙여넣기 경로도, `contenteditable` 화면도 전부 번들에서 떨어져 나갑니다. 표 첫 두 줄의 차이인 17킬로바이트가 그 값입니다.

`mawy-react/markdown`은 파서만 따로 가져오는 진입점입니다. 아무것도 그리지 않고 문서의 목차나 각주만 필요한 애플리케이션을 위한 것입니다. `mawy-react/server`는 브라우저로 **아무것도** 보내지 않습니다. 서버에서 문서를 그려 HTML로 내보내기 때문에, 표에 넣을 숫자가 없어서 줄이 없습니다.
