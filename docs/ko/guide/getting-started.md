...
::: tip `1.0.0`이고, 버전이 그렇게 말합니다

두 패키지 모두 `1.0.0`으로 게시되어 있습니다. 이 페이지의 내용은 전부 실제로 동작하고 — 이 사이트가 설치하는 것과 같은 소스에서 두 패키지를 모두 그립니다 — 여기서부터 내보내는 API는 유의적 버전을 따릅니다. 이름이 사라지거나 모양이 바뀌는 일은 메이저 버전을 기다립니다. 릴리스마다 무엇이 바뀌었는지는 [변경 기록](../changelog)에 있습니다.
...
React는 세지 않았습니다. 애플리케이션에 이미 있으니까요. `lucide-react`는 셌습니다. 이것 때문에 따라오니까요. **문서를 읽기만 하는 페이지는 에디터를 싣지 않습니다.** 툴바도, 실행 취소 기록도, 붙여넣기 경로도, `contenteditable` 화면도 전부 번들에서 떨어져 나갑니다. 위 두 줄 사이의 16킬로바이트가 그만큼의 값입니다.
...
- [**직접 써보기**](./playground) — 두 컴포넌트를 아무것도 끄지 않은 채로 놓아둔, 직접 입력해 보는 페이지.
