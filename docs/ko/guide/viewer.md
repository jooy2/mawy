...
**문서를 받을 수 없는 뷰어는 문서를 열자고 하지 않습니다.** `onValueChange` 없이 `value`만 넘긴 것은 문서가 애플리케이션의 것이라는 말이고, 여기서 고른 파일은 무엇도 될 수 없습니다. 그래서 빈 상태는 아직 보여줄 것이 없다고만 말하고, 그 아래 버튼은 그리지 않고, 툴바의 `open`은 비활성입니다. 눌러도 아무 일도 하지 않는 컨트롤은 없는 것만 못합니다.

:::

::: fw flutter

**마우스 휠 한 칸은 한 프레임이 아니라 몇 프레임에 걸쳐 도착합니다.** Flutter는 휠 한 칸에 대해 다음 프레임에서 스크롤 위치를 그 자리로 옮기고 그 사이는 아무것도 그리지 않습니다. 같은 문서를 브라우저에서 볼 때보다 Flutter에서 볼 때 손끝이 더 뻣뻣하게 느껴지는 이유가 그것입니다. 이 플랫폼들의 브라우저도 네이티브 애플리케이션도 전부 그 사이를 그립니다. 따로 켜거나 요청할 것은 없고, 움직임을 줄여 달라고 플랫폼에 말해 둔 사람에게는 다시 즉시 이동합니다. 스타일시트가 `prefers-reduced-motion`에서 내놓는 것과 같은 답입니다.

원문 편집 화면은 플랫폼의 것 그대로입니다. 텍스트 필드는 바깥의 무언가가 스크롤해 주는 것이 아니라 스스로 스크롤하고, 그 둘 사이에 설 자리가 없습니다.

**문서는 읽는 사람이 가져갈 수 있는 텍스트입니다.** 끌면 선택되고, `Ctrl`/`Cmd`+`C`로 선택한 것을 복사합니다. 이것을 굳이 적어 두는 이유는 공짜가 아니기 때문입니다. 위젯으로 그린 문서는 그렇다고 말해 주는 영역 안에 넣지 않으면 아무것도 선택되지 않는 화면이고, 위젯으로 그리는 것이 애초에 안전한 기본값을 공짜로 만들어 준 것입니다. 핸들도 컨텍스트 메뉴도 없습니다. 둘 다 Material이나 Cupertino의 것이고 이 패키지는 나머지 전부를 직접 그리니까요. 그래서 키는 `Enter`와 스페이스바를 적어 둔 것과 같은 이유로 적혀 있습니다.
...
**명세의 예제 652개 중 640개**, 그리고 그것을 바뀔 때마다 돌립니다. CommonMark는 테스트 모음이 안에 들어 있는 문서라서, "CommonMark를 읽는다"는 주장이 아니라 숫자입니다. 그 숫자는 `packages/react/test/internal/markdown/commonmark.test.ts`에 있고, 나머지 12개가 무엇인지도 그 옆에 적혀 있습니다.

그중 셋은 모자란 것이 아니라 결정입니다. 모든 URL은 스킴 허용 목록을 거치므로 `<made-up-scheme://foo>`는 작성자가 쓴 글자 그대로 그려집니다. 다섯은 같은 종류의 결정입니다. 목적지가 비어 있으면 `<a href="">` 대신 작성자가 쓴 낱말을 그립니다. 아무 일도 하지 않는 컨트롤이니까요. 나머지는 대체로 가장자리입니다. 목록 항목 안의 탭, 표에 없는 문자 참조, 목록 항목 안에 홀로 있는 참조 정의 같은 것들. 하나하나 왜 거기 있는지가 함께 적혀 있어서, 그 목록은 의도적으로만 짧아질 수 있습니다.
