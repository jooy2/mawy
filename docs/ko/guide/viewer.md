...
**블록이 사백 개를 넘는 문서는 보이는 곳만 만들어집니다.** 그 아래면 문서가 아무리 길어도 블록을 전부 만듭니다. 선택이 온전한 이유가 그것입니다. 선택은 이미 만들어진 글자만 가져갈 수 있습니다. 사백 개를 넘으면 읽는 사람이 스크롤할 때마다 블록이 만들어지고 사라지며, 문서를 그리는 비용과 글꼴이 바뀌어 다시 배치하는 비용과 메모리에 들고 있는 비용이 문서 크기를 따라 커지지 않습니다. 블록 이천사백 개짜리 문서에서 렌더 오브젝트는 이만 오천 개에서 오백 개로, 글꼴을 바꾼 뒤 다시 배치하는 시간은 1초에서 0.1초로 줄었습니다.

사백 개는 긴 README나 레퍼런스 한 쪽, 책의 한 장을 넘어서는 크기입니다. 그 위에서는 선택이 목록이 들고 있는 위아래 세 화면까지만 닿습니다. 툴바의 복사 버튼은 화면이 아니라 Markdown 원문에서 문서 전체를 가져가므로, 긴 문서를 통째로 복사하는 일은 선택을 거치지 않습니다. 켜고 끌 설정은 없습니다. 블록이 배치될 때마다 그 자리를 기록해 두며, 그 덕분에 아웃라인과 찾기 막대와 `anchors`가 화면에 없는 블록에 대해서도 계속 답할 수 있습니다.
...
**컨테이너는 100단까지 중첩됩니다.** 그 아래로는 아무것도 열리지 않습니다. 줄은 아무 규칙도 적용하지 않았을 때의 문단이 되고, 그 위에 있는 표시들은 그냥 글자가 됩니다. 컨테이너는 저마다 자기 안쪽을 다시 읽으므로 중첩은 문서 깊이만큼 쌓이는 호출입니다. `> `를 2천 번 쓴 4 kB짜리 파일은 안에 아무것도 없지만 스택을 바닥냅니다. 다른 데서 온 문서를 그리는 애플리케이션에 그런 파일을 보내는 것은 누구나 할 수 있습니다. 100단은 손으로 쓴 문서가 닿는 곳보다 훨씬 깊고, 어떤 스택보다도 얕습니다.
...
`'sanitize'`는 문서가 붙인 이름을 전부 `user-content-` 아래로 옮깁니다. `id`는 페이지의 전역이 됩니다. 어느 브라우저에서든 `<a id="config">`는 `window.config`입니다. `name`은 `document`에 같은 일을 하므로, `<img name="getElementById">`는 주위의 모든 스크립트에서 그 메서드를 빼앗습니다. 접두사 아래에서는 페이지가 가진 어떤 것과도 부딪히지 않습니다. 문서가 자기 이름으로 건 링크는 같이 옮겨지고, 제목으로 건 링크는 옮겨지지 않습니다. 제목의 앵커는 마크업이 아니라 작성자가 쓴 낱말이기 때문입니다. GitHub이 쓰는 것과 같은 낱말이라, GitHub용으로 쓴 문서는 그대로 동작합니다.
...
그래서 브라우저의 첫 페인트도 서버와 같은 답이고, 요소는 그다음 렌더에 나타납니다. 일부러 그렇게 했습니다. 곧바로 그리면 서버가 글자를 보낸 자리에서 React가 요소를 만나게 되고, 그것이 하이드레이션 불일치입니다. 서버에서 그리지 않는 애플리케이션은 하이드레이션도 하지 않으므로, 그쪽에서는 첫 렌더가 이미 브라우저의 것이라 미뤄지는 것도 깜빡이는 것도 없습니다.

`'raw'`는 아무것도 제거하지 않고 아무것도 검사하지 않습니다. 문서 안의 `<script>`가 실행되고, 이미지의 `onerror`가 실행되고, `<iframe>`이 로드됩니다. 전부 그 페이지의 출처에서, 그 페이지의 쿠키와 로그인한 독자가 닿을 수 있는 모든 것을 가지고 실행됩니다. **문서에 글자를 넣을 수 있는 사람은 애플리케이션이 할 수 있는 일을 전부 할 수 있습니다.** 남이 쓴 문서라면 그게 전부라는 뜻입니다. 세션을 읽고, 그 독자로 API를 부르고, 페이지를 바꿔 씁니다.

애플리케이션이 직접 쓴 문서이거나, 애플리케이션이 신뢰하는 무언가가 이미 안전하게 만들어 둔 문서에만 켜십시오. 이것을 켠 채 신뢰할 수 없는 마크다운을 그린 경우는 취약점 신고의 [범위 밖](https://github.com/jooy2/mawy/blob/main/SECURITY.md)입니다. 그것이 그 값의 문서화된 의미이기 때문입니다. 다른 데서 온 문서에 쓰는 값은 `'sanitize'`입니다.
...
**이미지가 도착할 자리를 미리 잡아 둘 수 있습니다.** Markdown에는 그림의 너비와 높이를 적을 자리가 없어서, 페이지가 얼마나 비워 둬야 할지 알 방법이 없습니다. 바이트가 도착하는 순간 그 아래가 전부 밀립니다. 문서 안에는 답이 없지만 바깥에는 있습니다. 애플리케이션이 자기 그림이 어떤 모양인지 안다면요.
...
그림이 오기 전에 자리를 잡고, `--mawy-doc-image-fit`은 `contain`이라 약속과 다른 모양의 그림은 잘리는 대신 그 안에 여백을 두고 들어갑니다. 지정하지 않으면 — 그것이 기본입니다 — 아무것도 달라지지 않고 아무 자리도 잡지 않습니다. 문서 안에 치수를 적는 것이 이 질문의 다른 답인데, 이 파서가 오늘 읽는 문법은 아닙니다.
...
- `Escape`를 누르면 툴팁이 사라지고, 포인터를 움직이면 다시 나옵니다. 사라져도 잃는 것은 없습니다. 툴팁이 이름 붙이는 버튼은 이미 같은 말을 스크린 리더에게 하고 있습니다.
- 인터페이스는 자기 말이 어느 언어인지 밝히고, 문서에 대해서는 밝히지 않습니다. 문서의 언어는 저자의 것이고 이 라이브러리가 아는 것이 아닙니다.
- `prefers-reduced-motion`에서는 애니메이션을 뺍니다.

**이 라이브러리가 직접 쓰는 제목 두 개는 `h2`이고, 바뀌지 않습니다.** 목차 패널의 제목과 빈 상태의 제목입니다. 문서 자신의 제목은 문서의 것이라, 뷰어를 어디에 두든 `# `는 `h1`입니다. 페이지의 제목 계층상 `h2`가 맞지 않는다면 바깥에서 다시 꾸미거나 다시 이름 붙이면 됩니다. 각각 `.mawy-outline-title`과 `.mawy-empty-title`이고, 단계를 받는 prop을 열어도 문서 자신의 단계는 저자가 둔 자리에 그대로 남습니다.
