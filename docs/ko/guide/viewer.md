...
여기에는 경계가 하나 있고, 문서가 지연 목록이기 때문에 생깁니다. 화면 근처의 블록만 만들어지므로 선택은 목록이 아직 들고 있는 글자까지만 가져갑니다. 위아래로 세 화면씩 남겨 두는데, 이는 한 번의 손짓으로 끄는 범위를 넘어섭니다. 그리고 툴바의 복사 버튼은 화면이 아니라 Markdown 원문에서 문서 전체를 복사하므로, 긴 문서를 통째로 가져가는 데에는 애초에 선택이 필요하지 않습니다.

**긴 문서는 보이는 곳만 만들어집니다.** 읽는 사람이 스크롤하면 블록이 만들어지고 사라집니다. 그래서 오천 줄짜리 문서를 그리는 비용, 글꼴이 바뀌어 다시 배치하는 비용, 메모리에 들고 있는 비용이 문서 크기가 아니라 한 화면 크기가 됩니다. 켜고 끌 설정은 없습니다. 블록이 배치될 때마다 그 자리를 기록해 두며, 그 덕분에 아웃라인과 찾기 막대와 `anchors`가 화면에 없는 블록에 대해서도 계속 답할 수 있습니다.
