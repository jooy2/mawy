...
이것이 전부입니다. 채워 넣을 테마 객체도, 문서를 그리기 전에 등록해야 할 것도, 렌더링을 맡길 두 번째 라이브러리도 없습니다.

## 왜 같은 패키지에 있나

::: fw flutter

그 사실은 두 패키지 사이에서도 성립하고, 주장이 아니라 검증된 것입니다. Dart 파서가 곧 TypeScript 파서입니다 — 같은 파일, 같은 함수, 같은 규칙 — 그리고 `packages/flutter/tool/parity.dart`가 저장소의 모든 마크다운 파일을 둘 다에 통과시켜 트리를 비교합니다. 브라우저에서 한 뜻인 문서는 앱에서도 같은 뜻입니다.

:::

쓰는 도구와 보여주는 도구가 서로 다른 구성에는, 벌어진 다음에는 반박하기 어려운 실패가 하나 있습니다. 작성자가 에디터에서 문서를 쓰고, 화면에서는 멀쩡해 보이는데, 독자에게는 다르게 그려지는 것입니다. 두 마크다운 구현 사이의 모든 차이 — 목록이 어떻게 중첩되는지, 줄바꿈이 줄바꿈인지, 닫히지 않은 강조가 어떻게 되는지 — 가 전부 그 실패의 씨앗입니다.
...
Flutter 패키지에는 없습니다. 여기서 `value`는 필수입니다. 파일을 여는 일은 파일 선택기를 뜻하고, 파일 선택기는 플러그인을 뜻합니다. 이 패키지에는 없고 애플리케이션에는 대개 이미 있는 의존성입니다. 그래서 파일을 읽는 것은 여러분의 몫이고 그리는 것이 Mawy의 몫이며, 이 절 전체가 React 패키지의 것입니다.

:::

::: fw react

`value`는 필수가 아니라 선택입니다. 편의를 위해 그렇게 둔 것이 아니라, 그것이 이 컴포넌트의 형태입니다. 문서가 없으면 뷰어 자체가 **파일 선택기**가 됩니다. `.md` 파일을 끌어다 놓거나, 골라서 열면 됩니다.
...
**문서를 받을 수 없는 뷰어는 문서를 열자고 하지 않습니다.** `onValueChange` 없이 `value`만 넘긴 것은 문서가 애플리케이션의 것이라는 말이고, 그러면 여기서 고른 파일은 아무것도 될 수 없습니다. 그래서 빈 상태는 아직 보여줄 것이 없다고만 말하고, 그 아래 버튼은 그리지 않고, 툴바의 `open`은 비활성입니다. 눌러도 아무 일도 하지 않는 컨트롤은 없는 것만 못합니다.

:::

::: fw flutter

**마우스 휠 한 칸은 한 프레임이 아니라 몇 프레임에 걸쳐 도착합니다.** Flutter는 원래 휠 한 칸을 받으면 다음 프레임에서 스크롤 위치를 그 자리로 옮기고, 그 사이는 아무것도 그리지 않습니다. 같은 문서를 브라우저에서 볼 때보다 Flutter에서 볼 때 손끝이 더 뻣뻣하게 느껴지는 이유가 그것입니다. 이 플랫폼에서는 브라우저도 네이티브 애플리케이션도 모두 그 사이를 그립니다. 따로 켜거나 요청할 것은 없고, 움직임을 줄여 달라고 플랫폼에 말해 둔 사람에게는 다시 즉시 이동합니다. 스타일시트가 `prefers-reduced-motion`에서 내놓는 것과 같은 답입니다.

원문 편집 화면은 플랫폼의 것 그대로입니다. 텍스트 필드는 바깥의 무언가가 스크롤해 주는 것이 아니라 스스로 스크롤하고, 그 둘 사이에 설 자리가 없습니다.

**문서는 읽는 사람이 가져갈 수 있는 텍스트입니다.** 끌면 선택되고, `Ctrl`/`Cmd`+`C`로 선택한 것을 복사합니다. 이것을 굳이 적어 두는 이유는 거저 되는 일이 아니기 때문입니다. 위젯으로 그린 문서는 선택할 수 있다고 말해 주는 영역 안에 넣지 않으면 아무것도 선택되지 않습니다. 그 대신 위젯으로 그리기 때문에 안전한 기본값은 거저 따라옵니다. 핸들도 컨텍스트 메뉴도 없습니다. 둘 다 Material이나 Cupertino의 것이고, 이 패키지는 나머지 전부를 직접 그리기 때문입니다. 그래서 이 단축키도 `Enter`와 스페이스바를 적어 둔 것과 같은 이유로 적어 둡니다.
...
**명세의 예제 652개 중 640개**를 통과하고, 코드가 바뀔 때마다 그 검사를 돌립니다. CommonMark는 테스트 모음이 안에 들어 있는 문서라서, "CommonMark를 읽는다"는 주장이 아니라 숫자입니다. 그 숫자는 `packages/react/test/internal/markdown/commonmark.test.ts`에 있고, 나머지 12개가 무엇인지도 그 옆에 적혀 있습니다.

그중 셋은 모자란 것이 아니라 결정입니다. 모든 URL은 스킴 허용 목록을 거치므로 `<made-up-scheme://foo>`는 작성자가 쓴 글자 그대로 그려집니다. 다섯은 같은 종류의 결정입니다. 목적지가 비어 있으면 `<a href="">` 대신 작성자가 쓴 낱말을 그립니다. 아무 일도 하지 않는 컨트롤이기 때문입니다. 나머지는 대체로 가장자리입니다. 목록 항목 안의 탭, 표에 없는 문자 참조, 목록 항목 안에 홀로 있는 참조 정의 같은 것입니다. 하나하나 왜 거기 있는지를 함께 적어 두었으므로, 그 목록은 일부러 줄일 때만 줄어듭니다.
...
주석 하나가 여러 블록일 수도 있습니다. 두 번째 문단이든 목록이든 코드 블록이든, 첫 줄 다음부터 네 칸씩 들여쓰기만 하면 됩니다. 어디에 썼는지는 상관없습니다. 파서가 흐름에서 들어내므로, 문서 한가운데에 쓴 주석도 아래에서 읽힙니다.

### 정의 목록

여기서 GitHub이 읽지 않는 유일한 것입니다. 문법은 [PHP Markdown Extra](https://michelf.ca/projects/php-markdown/extra/#def-list)의 것이고, 이런 목록을 쓰는 사람들이 실제로 쓰는 바로 그 문법입니다.
...
용어는 글줄 하나이고, 그 뜻은 콜론 **하나와 공백**으로 시작하는 줄입니다. 그 공백이 없으면 문장 아래에 쓴 `:warning:`이 그 문장을 용어로 만들어 버립니다. 아주 많은 문서가 거기에 걸립니다. 여러 용어가 한 뜻을 나눠 가질 수도, 한 용어가 여러 뜻을 가질 수도 있고, 첫 줄 이후를 들여쓰면 뜻 하나가 여러 블록일 수도 있습니다. 뜻 앞의 빈 줄은 목록 전체를 성기게 만듭니다. 글머리 기호 목록에서와 똑같습니다.

`parse`에 `definitionLists: false`를 넘기면 꺼집니다. GitHub에서와 정확히 같은 뜻이어야 하는 문서를 위한 스위치입니다.

## 코드 블록에 색 입히기

::: fw flutter

기본값은 색을 입히지 않는 것이고, 빠뜨린 것이 아닙니다. 하이라이터는 마크다운 렌더러가 짊어질 수 있는 것 중 가장 크고, 대부분의 문서에는 색을 입힐 것이 아예 없습니다. 그래서 인자로 두었고, 하이라이터를 하나도 이름 대지 않은 애플리케이션은 그 뒤의 문법 표를 아예 싣지 않습니다. Dart 빌드가 아무도 참조하지 않는 코드를 떨어뜨리기 때문입니다.
...
언어는 문서가 흔히 보여주는 것들입니다. `js`, `ts`, `jsx`, `tsx`, `json`, `html`, `xml`, `css`, `bash`, `python`, `yaml`, `sql`, `dart`, `go`, `rust`, `java`, `c`, `cpp`, 그리고 각각이 함께 답하는 다른 이름들입니다. 이 결과는 일부러 **근사치**로 두었고 앞으로도 그렇습니다. 중괄호가 든 템플릿 리터럴이나 나눗셈처럼 읽히는 정규식은 조금 어긋나게 나오지만, 그것은 문제가 되지 않습니다. 색은 반드시 맞아야 하는 종류의 답이 아니기 때문입니다.
...
기본값은 색을 입히지 않는 것이고, 빠뜨린 것이 아닙니다. 하이라이터는 마크다운 렌더러가 짊어질 수 있는 것 중 가장 크고, 대부분의 문서에는 색을 입힐 것이 아예 없습니다. 그래서 프롭으로 두었고, 그 프롭은 **함수**를 받습니다. 펜스에 언어가 적힌 문서를 실제로 그리기 전까지는 가져오지도 않게 하기 위해서입니다.
...
`mawy-react/highlight`는 별도의 진입점이라, 언급한 적 없는 애플리케이션의 번들에는 들어가지 않습니다. 안에 든 것은 Mawy가 직접 만든 하이라이터이고, 문서가 흔히 보여주는 언어들 — `js`, `ts`, `jsx`, `tsx`, `json`, `html`, `xml`, `css`, `bash`, `python`, `yaml`, `sql`, `dart`, `go`, `rust`, `java`, `c`, `cpp`과 그것들이 함께 답하는 이름들 — 을 압니다. 이 결과는 일부러 **근사치**로 두었고 앞으로도 그렇습니다. 중괄호가 든 템플릿 리터럴이나 나눗셈처럼 읽히는 정규식은 조금씩 틀리게 나오지만, 그것은 문제가 되지 않습니다. 색은 반드시 맞아야 하는 종류의 답이 아니기 때문입니다.
...
뷰어는 마크다운이 말할 수 있는 것을 그립니다. 그런데 문서는 가끔 마크다운에 낱말이 없는 것을 말하고 싶어 합니다. 영상, 수식, 사이트의 모든 페이지가 쓰는 우리 회사 알림 상자 같은 것입니다. 여기서 혼자 빠져나가는 길은 둘뿐이고 둘 다 나쁩니다. 하나는 원시 HTML인데, 이 라이브러리의 안전 이야기 전체가 그것을 쓰지 않는다는 데 서 있습니다. 다른 하나는 영상을 아는 라이브러리인데, 그러면 그다음에는 모든 것을 알아야 합니다.
...
컴포넌트가 받는 것은 `name`, `attributes`, 이미 그려진 `label`, 컨테이너라면 이미 그려진 `children`, 그것이 쓰인 자리인 `range`, 그리고 쓰인 글자인 `source`입니다. 그래서 컴포넌트는 React 엘리먼트를 조립할 뿐 마크업 문자열을 볼 일이 없습니다. 확장 지점이 생겨도 안전 이야기가 그대로인 이유가 그것입니다. 마크다운과 화면 사이에 여전히 `innerHTML`이 없고, 위험한 것을 그리는 디렉티브는 애플리케이션이 그것을 그린 것입니다.
...
**링크는 새 탭에서 열립니다.** `linkTarget`의 기본값이 `'blank'`이고, 그러면 링크에 `target="_blank"`와 `rel="noopener noreferrer"`가 붙습니다. 뷰어는 대개 페이지 그 자체가 아니라 페이지의 한 조각입니다. 링크를 따라 나갔다 돌아온 사람은 문서를 두고 간 자리에서 다시 만나야 하고, 에디터라면 그 링크 뒤에 저장하지 않은 글이 있습니다. `linkTarget="self"`는 문서를 페이지 _그 자체로_ 보여주는 애플리케이션을 위한 것입니다. 각주 번호와 거기서 돌아오는 화살표는 같은 페이지를 가리키므로 어느 쪽이든 영향을 받지 않습니다.
...
**링크를 대신 열지도 않습니다.** 탭한 링크는 `onLinkTap`으로 애플리케이션이 "연다"는 것이 무엇인지 말하기 전까지 아무 일도 하지 않습니다. URL을 플랫폼에 넘기는 것은 뷰어가 할 결정이 아닙니다. 스킴 허용 목록은 이미 지나온 뒤이고, 나머지는 여러분 몫입니다.
...
`'sanitize'`는 정규식이 아니라 `DOMParser`로 파싱하고, 이는 의도적입니다. HTML의 오류 복구가 곧 공격면이고, `<img src=x onerror=alert(1)>`을 브라우저와 똑같이 해석하는 파서는 브라우저의 것뿐입니다. `DOMParser`가 없는 곳 — 서버 렌더링 — 에서는 추측하는 대신 마크업을 텍스트로 보여줍니다.
...
툴바는 문서가 무엇을 말하는지가 아니라 문서를 어떻게 **조판할지**를 다룹니다. 독자가 글자를 키우고, 숨 쉴 자리를 넓히고, 명조로 바꿔도 아래의 문서는 그대로입니다.
...
목록에 없는 컨트롤을 얹을 방법은 없고, 이는 의도적입니다. 아무 자식이나 받는 툴바는 라이브러리가 더 이상 키보드로 다루게 만들어 줄 수 없는 툴바입니다.
...
모든 컨트롤은 이름이 있고 눌린 상태를 말하는 `Semantics` 버튼입니다. 스크린 리더가 도형의 줄이 아니라 툴바를 읽습니다. React 패키지의 것처럼 탭 정지점은 하나이고, 안에서는 화살표가 컨트롤 사이를 옮깁니다. 아래 [접근성](#접근성)을 보세요.

:::

## 글꼴

기본으로 제공하는 것은 셋이고, 글꼴 이름이 아니라 역할입니다. `sans`·`serif`·`mono`를 독자의 기기에 이미 있는 글꼴로 그립니다. 내려받는 것이 없으니 실패할 것도 없습니다.
...
모든 조판 값은 `--mawy-doc-*` 커스텀 속성으로 화면에 도달합니다. 그래서 들어가는 길이 둘이고, 둘은 결국 같은 길입니다.

하나는 프롭으로 넣고 뷰어가 갖고 있게 하는 길입니다.
...
다른 하나는 툴바를 아예 끄고 CSS로 직접 정하는 길입니다.
...
`defaultTypography`는 뷰어를 어딘가에서 시작하게 하고, 설정은 뷰어가 갖고 있게 둡니다. `typography`를 넘기면 그 설정을 애플리케이션이 가져갑니다. 이때도 툴바는 독자가 고른 것을 `onTypographyChange`로 알려주고, 애플리케이션이 새 값을 되돌려 주기 전까지는 아무것도 움직이지 않습니다. `onTypographyChange`는 어느 쪽이든 호출되므로, 독자의 선택을 기억하는 코드는 양쪽에서 같은 코드입니다.
...
**영어와 한국어**이고, 기본값은 `en`입니다. 문서에 대해서는 아무 말도 하지 않습니다. 인터페이스가 영어인 뷰어에 한국어 문서가 들어 있는 것은 평범한 경우이고, 둘은 일부러 분리된 질문입니다.
...
다시 선언하는 일에 해당하는 것이 `tokens`입니다. 팔레트가 아니라 밝기를 받는 함수인데, 뷰어는 나머지를 다 받은 다음에야 자기 밝기를 정하기 때문입니다. `colorScheme`에서 정하고, 그 값이 `system`이면 플랫폼에서 정합니다. 플랫폼을 따라가는 문서라면 처음 열린 팔레트에서만이 아니라 두 팔레트 모두에서 따라가야 합니다. 서른한 개의 색을 다시 쓰지 않고 하나를 만드는 방법이 `copyWith`입니다.
...
`MawyRadius`와 `MawyMotion`은 모서리 반경, 그리고 움직이는 모든 것이 쓰는 하나의 시간과 하나의 곡선에 같은 생각을 적용한 것입니다.
...
`:root`가 아니라 `.mawy-root`에 있는 것은 의도적입니다. 컴포넌트 라이브러리가 문서 루트에 값을 쓸 일은 없고, `:root`에서 팔레트를 읽는 뷰어는 밝은 페이지 안에서 어두울 수 없습니다. 문서 하나를 끼워 넣을 때 흔히 원하는 것이 바로 그것인데도 말입니다.
...
뷰어가 그린 모든 엘리먼트는 `data-mawy-range="start,end"`를 갖습니다. 넘겨받은 마크다운에서 그 조각의 첫 글자와 마지막 글자 다음의 위치입니다. 블록, 목록 항목, 표의 행과 셀, 그리고 그 안의 인라인 엘리먼트 — 강조, 링크, 코드 스팬, 이미지 — 가 모두 하나씩 갖습니다.

코드 블록은 두 번 말합니다. 바깥의 상자는 펜스와 정보 문자열과 들여쓰기까지 통째로 가리키고, 안의 `code` 엘리먼트는 코드만 가리킵니다. 커서가 놓일 수 있는 것은 뒤쪽이고, 펜스 사이에 아무것도 없어도 자리는 자리입니다. 그때는 두 위치가 같은 수입니다.
...
범위는 화면의 어떤 자리에서 그 자리가 그려져 나온 문서의 자리로 돌아가는 유일한 길입니다. 에디터의 `split`은 이것을 두 번 읽습니다. 원문 맨 위 줄이 속한 블록으로 미리보기를 스크롤할 때, 그리고 미리보기에서 클릭한 단어로 커서를 옮길 때. 애플리케이션도 같은 종류의 일에 쓸 수 있습니다 — 문단에 달린 댓글, 제목 옆의 "이 절 편집" 버튼. 오프셋은 넘긴 문자열을 UTF-16 코드 유닛으로 직접 가리키므로, `value.slice(start, end)`가 곧 클릭된 것 뒤의 마크다운입니다.

화면에서 범위가 붙지 않는 것은 텍스트 하나뿐입니다. 속성을 담을 자리가 없기 때문입니다. 그럴 필요도 없습니다. 텍스트 한 줄기는 양옆의 엘리먼트로 둘러싸여 있고, 그 사이에서 원문을 찾기에는 그것으로 충분합니다. `**bold**`로 그려진 `<strong>` 안에서 `bold`가 있는 자리는 그 여덟 글자 중 정확히 한 곳입니다.
...
코드가 바뀔 때마다 모든 화면을 [axe](https://github.com/dequelabs/axe-core)로 훑습니다. 문서를 띄운 뷰어와 파일 선택 상태의 뷰어, 에디터의 네 가지 모드, 목차를 연 상태, 메뉴를 연 상태, 찾기와 바꾸기를 연 상태를 각각 두 팔레트 모두에서 훑습니다. 거기서 나온 것은 위 목록에 이미 들어 있습니다. 체크 목록의 체크박스는 옆의 글이 이름이 되고, 팔레트에서 가장 옅은 글자색은 두 배경 모두에서 AA를 넘도록 올렸고, 편집 화면은 `article`이 아니라 `div`입니다. ARIA는 문서 구획이 `textbox`가 되는 것을 허용하지 않기 때문입니다. 각주에서 본문으로 돌아가는 링크에는 밑줄이 생겼습니다. 색만으로 구분되던 링크였습니다.
...
- 메뉴는 포커스를 안에 넣은 채로 열리고, `Escape`로 닫히며, 열었던 버튼으로 포커스를 돌려줍니다. React 패키지와 달리 그렇게 열 수밖에 없습니다. 패널은 `Overlay`로 띄우므로 `Tab`이 걷는 순서에서 자기 버튼 근처에 있지 않고, 거기까지 가려고 문서 전체를 지나야 하는 키보드는 가지 않습니다.
- 목차의 항목은 저마다 탭 정지점입니다. 툴바처럼 정지점 하나에 화살표가 아니라, React 패키지에서 `<ol>` 안의 `<button>`들이 그렇듯 `Tab` 한 번에 하나씩입니다. 제목 여섯 개짜리 줄은 움직이는 방법을 하나 더 익힐 만한 줄이 아닙니다.
- 제목·링크·이미지는 문서 안에서 각자의 시맨틱을 들고 있고, 목차 항목을 따라가면 스크롤뿐 아니라 포커스도 함께 갑니다. 다음 `Tab`이 패널이 아니라 제목에서 이어집니다. 제목 자체는 탭 정지점이 아닙니다. `skipTraversal`이고, 웹의 `tabIndex = -1`을 반대로 말한 것입니다. 포커스를 _놓을_ 수 있는 자리이지 `Tab`이 지나가다 멈추는 자리가 아닙니다.
- 글자 크기는 플랫폼의 크기 설정을 따릅니다. 크기가 어딘가에 박혀 있지 않고 `MawyTypography`를 통과하는 논리 픽셀이기 때문입니다.
- 플랫폼의 모션 줄이기 설정이 켜져 있으면 애니메이션을 뺍니다. `MediaQuery.disableAnimationsOf`이고, 스타일시트가 `prefers-reduced-motion`으로 묻는 것과 같은 질문입니다. 그것을 켜 둔 독자는 두 패키지 모두에서 같은 것을 받습니다.
...
나머지 셋은 화면이 아니라 페이지에 관한 것입니다. 높이를 받은 뷰어는 그 안에서 스크롤하는데, 높이가 있는 상자는 한 상자 분량만 인쇄하고 나머지를 잃습니다. 종이 위에서는 문서만큼 깁니다. 팔레트는 독자가 무엇을 골랐든 밝은 바탕에 어두운 글자가 됩니다. 어두운 테마를 인쇄하면 흰 글자를 두른 잉크 사각형이기 때문입니다. 그리고 링크의 목적지는 링크 뒤에 적힙니다. 아무것도 가리키지 않는 "문서를 보세요"는 요점을 잃은 문장이기 때문입니다.
