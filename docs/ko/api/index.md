...
두 패키지 모두 `1.0.0`으로 게시되어 있습니다. 여기서부터 이 페이지의 이름들은 유의적 버전을 따릅니다. 이름이 사라지거나 모양이 바뀌는 일은 메이저 버전을 기다립니다.
...
이름은 파일을 열었다면 그 파일의 것이고, 아니면 문서의 첫 제목입니다. 에디터에 놓은 파일은 기본적으로 문서가 아니라 이미지입니다 — 이유와 `fileDrop`으로 그 규칙을 끄는 법은 [열기와 저장](../guide/editor#열기와-저장)에 있습니다.
...
**영어와 한국어**이고 기본값은 `en`입니다. 뷰어와 에디터 자신의 chrome이 쓰는 언어 — 툴바 레이블, 메뉴 항목, 스크린 리더에게 주는 문장 — 이고, 문서가 쓰인 언어와는 무관합니다. 두 패키지가 같은 이름 아래 같은 낱말을 싣습니다. 한쪽에만 있는 언어는 이 라이브러리가 가진 언어가 아닙니다.
...
`find`는 문서 위에 찾기 바를 열고, 뷰어에 포커스가 있는 동안 `Ctrl`+`F`(`Cmd`+`F`)를 가져갑니다. 빼면 단축키는 다시 브라우저의 것이 되고, 페이지 전체를 채우는 뷰어라면 그편이 맞습니다 — 이건 자기 영역 안에 들어앉은 뷰어를 위한 것이고, 그런 뷰어는 브라우저의 찾기가 안으로 들어오지 않고 지나쳐 갑니다. 찾는 대상은 문서가 _그리는_ 글자입니다. `**bold**` 안의 `bold`는 찾히고 `**`는 아무것도 찾히지 않습니다. 한 매치가 두 런에 걸칠 수는 없어서 `he**llo**`에서 `hello`는 찾히지 않고, 코드 블록은 검색하지 않습니다.
