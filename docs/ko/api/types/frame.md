...
- `floating` — 문서를 감싸는 것이 없습니다. 배경이 없고, 툴바는 글 위에 뜬 둥근 바가 됩니다. 휴대폰이 컨트롤을 대상 위에 올려놓는 방식과 같습니다. 문서가 곧 페이지인 경우를 위한 값입니다. 글, 게시물, README처럼 본문을 감싼 상자가 화면 전체를 감싼 상자가 되어 아무 말도 하지 않는 자리입니다.
