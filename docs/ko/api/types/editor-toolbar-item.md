...
`table`은 표를 넣고 모양을 바꾸는 명령 일곱 개를 담은 메뉴이고, 명령마다 단축키가 따로 있습니다. [표](../../guide/editor#표)를 보세요.
...
`open`은 `onOpen`을 줬을 때만 그립니다. 파일 선택기는 플러그인이고, 버튼을 눌렀을 때 무엇을 열지는 애플리케이션이 정하기 때문입니다. `save`는 없습니다. 애플리케이션이 `onChange`로 문서를 받아 어디에 둘지 정합니다. [열기와 저장](../../guide/editor#열기와-저장)을 보세요.
