...
| `imageAddress` | `boolean` | `true` | 그려진 문서에서 그림 옆 막대에 주소를 보여 줄지. |
...
문서에서 그림이 빠져도 아무것도 부르지 않습니다. 실행 취소하면 그림이 돌아올 수 있기 때문입니다. 대신 [`imageUrls`](../functions/image-urls)가 저장된 문서가 가리키는 그림을 알려 주고, 그 페이지에 이유가 있습니다.

:::

::: fw flutter

여기에는 이미지 업로드가 없습니다. 에디터는 애플리케이션이 건넨 URL로 마크다운을 쓸 뿐이고, 파일을 고르는 일은 위젯이 아니라 플러그인의 몫입니다. 저장해 둔 파일 가운데 더는 쓰이지 않는 것을 찾으려면 [`imageUrls`](../functions/image-urls)로 저장된 문서가 가리키는 그림을 얻으면 됩니다.
