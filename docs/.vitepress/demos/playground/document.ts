...
This is the editor from the package — not a picture of it, and not a cut-down copy. What you type changes the Markdown, and the Markdown is what every other surface is a view of.

## The switch at the left

Four answers to one question: \`wysiwyg\` draws the document and edits it where it is drawn, \`plain\` is the source, \`preview\` is the reading, and \`split\` is both. In \`split\` the bar between the panes is something to take hold of — drag it, or give it the focus and press the arrows.

## The rest of the toolbar

Narrow the window and the buttons leave from the end, a group at a time, into the menu marked with an ellipsis. Nothing wraps and nothing is lost.
...
> [!TIP]
> Drop an image file on the editor, or paste a screenshot. This page answers \`onUploadImage\` with a \`data:\` URI, because there is no server behind a documentation site — a real application answers with wherever it put the bytes.

:::callout[A directive this page declared]{kind=note}
\`callout\`, \`progress\` and \`kbd\` are the site's rather than the library's. The parser reads the shape; what the shape means is whoever embedded the editor.
:::
...
창을 좁히면 버튼이 끝에서부터 그룹 단위로 빠져나가 말줄임표 메뉴로 들어갑니다. 줄바꿈도 없고 사라지는 것도 없습니다.
...
> [!TIP]
> 이미지 파일을 에디터에 끌어다 놓거나 스크린샷을 붙여넣어 보세요. 이 페이지는 \`onUploadImage\`에 \`data:\` URI로 답합니다. 문서 사이트 뒤에는 서버가 없기 때문이고, 실제 애플리케이션이라면 바이트를 올려둔 곳의 주소로 답합니다.

:::callout[이 페이지가 직접 선언한 디렉티브]{kind=note}
\`callout\`, \`progress\`, \`kbd\`는 라이브러리의 것이 아니라 이 사이트의 것입니다. 파서는 모양까지만 읽고, 그 모양이 무엇을 뜻하는지는 에디터를 가져다 쓴 쪽이 정합니다.
:::
