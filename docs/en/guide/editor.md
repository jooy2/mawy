...
**A list given up in the middle is left loose until something is typed there.** `Enter` on an empty item between two others takes the marker away and leaves the blank line, and one blank line does not end a list: `- one`, nothing, `- two` is one loose list to CommonMark, and one loose list is what the preview, the viewer and GitHub all draw. While the caret is on that blank line the surface draws the two lists and the empty paragraph the first letter typed will make, so there is somewhere to type; move the caret away without typing and the list is drawn as the loose list the source says it is, which is a wider gap than the one it had. `Backspace` on that paragraph puts the list back together, `Delete` does the same from the other side, and so does undo. The blank line is not taken back on its own, because it is an edit that was made and a caret moving is not a keystroke.
