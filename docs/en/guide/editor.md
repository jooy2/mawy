...
Inside a code block none of it happens, because everything in there is the characters it is. A fence typed in there is three characters somebody wants in their code, so rather than closing the block it makes the block's own fences longer: a block holding a run of three is written with four, which is what the notation has for exactly this and what every file showing Markdown inside Markdown is written with. The same goes for a fence pasted in.
