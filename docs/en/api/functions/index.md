...
What the package exports that is not a component or a type: the parser on its own, the anchors it gives a heading, the highlighter, and the arithmetic behind the find bar.
...
`parseMarkdown` and `slugify` live in `mawy-react/markdown`, and `mawyHighlighter` in `mawy-react/highlight`. Both are entry points of their own, so an application that never references one never ships it — and neither needs React or a DOM, so the parser runs in a build script or on a server as readily as on a page.
