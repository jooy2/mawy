...
- **A footnote's number goes to the note, and the note goes back.** The two halves of a footnote have pointed at each other since footnotes were read at all, and in the React package they are two links the page follows. There is nowhere for a link to point here: a span has no place of its own and the notes are drawn under the document rather than being part of it. So the viewer scrolls, which is the answer the outline and the find bar already arrive at. Tapping the number brings the note it points at into view; the arrow at the end of the note — new, and named `Back to where this was mentioned` for a screen reader — brings back the block the footnote was first mentioned in.
