...
copy. What you type changes the Markdown, and the Markdown is what the other
surface is a view of.

## The switch at the left

Three answers to one question: `plain` is the source, `preview` is the reading,
and `split` is both. There is no `wysiwyg` here, and the editor guide says why:
that surface edits the document where it is drawn, which rests entirely on
`contenteditable`, and an `EditableText` owns a string.
...
Narrow the window and it scrolls sideways rather than putting its end in a menu,
which is the one place this package and the React one answer differently. A row
that slides is what a finger already knows to do; a menu is what a pointer does.

- [x] `Mod` + **B**, *I*, K and E do what the buttons do
- [x] `Mod` + **F** opens the find bar over the source, with replace beside it
- [ ] Opening and saving, which are the application's here — a file picker is a
      plugin rather than a widget
...
> [!TIP]
> Drag across the source to select, double tap for a word, triple tap for a
> line. Then press a toolbar button: every command acts on the selection.

:::callout[A directive the gallery declared]{kind=note}
`callout`, `progress` and `kbd` are the gallery's rather than the library's. The
...
놓은 사본도 아닙니다. 여기에 입력하는 것이 곧 마크다운이고, 옆의 화면은 그
마크다운을 보는 방법입니다.

## 왼쪽 끝의 전환

하나의 질문에 대한 세 가지 답입니다. `plain`은 원문, `preview`는 읽기,
`split`은 둘 다입니다. `wysiwyg`는 여기에 없고, 그 이유는 에디터 문서에
적혀 있습니다. 그 화면은 그려진 문서를 그 자리에서 고치는 것이고 그것은 전적으로
`contenteditable` 위에 서 있는데, `EditableText`는 문자열 하나를 가질 뿐입니다.

`split`에서 가운데 막대는 잡고 움직이는 것입니다. 끌어도 되고, 포커스를 준 뒤
방향키를 눌러도 됩니다.

## 툴바의 나머지

창을 좁히면 툴바는 메뉴로 접히는 대신 옆으로 스크롤합니다. 이 패키지와 React
패키지가 다르게 답한 유일한 자리입니다. 손가락은 밀리는 줄을 이미 알고 있고,
메뉴는 포인터가 하는 일입니다.

- [x] `Mod` + **B**, *I*, K, E는 버튼과 같은 일을 합니다
- [x] `Mod` + **F**는 원문 위에 찾기 막대를 엽니다. 바꾸기도 그 옆에 있습니다
- [ ] 열기와 저장은 여기서는 애플리케이션의 몫입니다. 파일 선택기는 위젯이
      아니라 플러그인이기 때문입니다
...
> [!TIP]
> 원문 위를 끌면 선택되고, 두 번 누르면 단어가, 세 번 누르면 줄이 잡힙니다.
> 그런 다음 툴바 버튼을 눌러 보세요. 모든 명령은 선택된 것에 작용합니다.

:::callout[갤러리가 직접 선언한 디렉티브]{kind=note}
`callout`, `progress`, `kbd`는 라이브러리의 것이 아니라 갤러리의 것입니다.
