...
///
/// The first one, and the two the playground uses, are the documentation site's
/// own — word for word, in both languages — so that the React preview and the
/// Flutter preview on one page are showing the same file. Two documents that
/// only look alike would hide the disagreement they exist to find.
library;
...
class Sample {
  ...
  ///
  /// Only the playground has one, because the playground is the page a reader
  /// stays on and types into, and prose in a language they did not pick is
  /// prose they will not read a line of before clearing it. Every other sample
  /// is a specimen: what a table or a footnote looks like does not depend on
  /// who is reading it.
  final String? korean;
...
The head has a second spelling as well, which is what every tool that shipped containers before the proposal writes. `MawyCodeGroup` is the one drawing this package ships, registered here under two names. Several blocks in one group are tabs, named by the language each fence was written with, or by the name a fence wrote in brackets after it:
...
/// The playground's document, which both of its samples show.
///
/// The site's `demos/playground/document.ts` holds these same two strings and
/// says at length why: the editor opens on it, the viewer beside it reads it,
/// and everything the parser knows how to read is in it once, in the place a
/// handbook for a corner of a street would really have used it.
///
/// The two pictures live in the site's `public/sample` and are asked for from
/// the root, so they arrive in the framed preview and not in this gallery run
/// on its own — where the viewer draws the alt text instead, which is what it
/// draws for any picture it cannot fetch. Copying the files here would put the
/// same bytes in the published package for the sake of a preview.
const String _playground = '''
# Maple Court rain garden
...
const List<Sample> samples = <Sample>[
  ...
  Sample(
    ...
    'Playground editor',
    _playground,
    korean: _playgroundKo,
    editor: true,
  ),
  Sample('playground/viewer', 'Playground viewer', _playground, korean: _playgroundKo),
];
