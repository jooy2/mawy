...
> [!IMPORTANT]
> **The parser, the viewer and the editor are written and tested.** `wysiwyg` is
> the one surface this package does not have, and [the guide says
> why](https://mawy.cdget.com/guide/editor). The exported API is under semantic
> versioning: a name that goes away or changes shape waits for a major version.
