...
- **Typing in `wysiwyg` costs the size of the edit rather than the length of the document.** The document is parsed and drawn again under the caret on every keystroke, and two things in that path read the whole page each time: finding where the caret goes looked at every element, and backspace at the start of a block collected every run of text in the file to find the one before it. An element says which characters it was drawn from, so nothing inside one that misses the caret can hold it — those are stepped over whole now — and the run on the other side of a block is stepped to rather than gathered. What each of them walks is the way to the block being typed in.
