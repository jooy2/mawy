...
- **A name a document gives something no longer reaches the page around it.** With `html="sanitize"`, an `id` and an `<a name>` written in the document went onto the page as written — and both of those become properties: `<a id="config">` is `window.config` in every browser, and `<img name="getElementById">` takes that method away from every script on the page. Neither needs a mistake by the application. Names a document writes go under `user-content-` now, which is the word GitHub uses, so a document written for GitHub keeps working. Links the document wrote to its own names move with them; a link to a heading does not, because a heading's anchor is the author's words rather than markup, and a table cell's `headers` moves with the header cells it names.
...
- **Reading a long document does less work per character of it.** The reader that tidies a document before the block rules see it built each line by adding one character to a string at a time, which is a string made and thrown away for every character in the file. Lines are sliced out whole now. Nothing about what is parsed changes; both parsers still produce the same tree, character for character.

- **Typing in `wysiwyg` costs the size of the edit rather than the length of the document.** The document is parsed and drawn again under the caret on every keystroke, and three things in that path did more than the edit asked for. Finding where the caret goes looked at every element on the page; backspace at the start of a block collected every run of text in the file to find the one before it; and the surface's own `beforeinput` and `paste` listeners came off the element and went back on for every character, because what they answer with is the document. An element says which characters it was drawn from, so nothing inside one that misses the caret can hold it — those are stepped over whole now — the run on the other side of a block is stepped to rather than gathered, and the listeners are bound once and read the document where it is.
