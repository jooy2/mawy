...
- **Every block element says where it came from.** The viewer draws each block with `data-mawy-range="start,end"` — the offsets in the Markdown it was given of that block's first character and of the one after its last — and list items and table rows carry it too. The offsets index the string that was passed, so `value.slice(start, end)` is the Markdown behind whatever was clicked. It is a range because a range is the only way back from a place on the page to the place in the document it was drawn from.
