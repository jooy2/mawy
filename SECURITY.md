...
- What an attacker can do with it — reading data, running script, escaping into the embedding page — rather than only what the input looks like.
...
If you do not hear back inside those windows, please chase the report rather than assume it was received.
...
- **Denial of service in the parser.** An input of reasonable size that makes parsing or rendering take unbounded time or memory — pathological backtracking, unbounded nesting, quadratic blow-up.
...
We ask for coordinated disclosure: give us a chance to release a fix before the details are public. In return, we will not sit on a report — the timetable above is what we hold ourselves to, and if a fix will take longer than that, we will say so and why.
